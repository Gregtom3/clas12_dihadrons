...
declare -a hipodirs=("/cache/clas12/rg-a/production/recon/fall2018/torus-1/pass1/v1/dst/train/nSidis/")
...
for dir in "${hipodirs[@]}"; do
    ...
    for ((i=0; i<$file_count && i<${#files[@]}; i++)); do
        file_path=${files[i]}
        read runNumber <<< $(basename $file_path | grep -oP '(?<='nSidis'_00).*(?=.hipo)')
        
        hipo_file=$file_path
        root_file="nSidis_RGA_${runNumber}.root"
...
EOF
		...
		j=$((j+1))
            ...
            echo "clas12root -b -q $PWD/macros/hipo2tree_multiplicity_data.C\(\\\"${hipo_file}\\\",\\\"${outfile}\\\",$beamE,$pid1,$pid2,$maxEvents\)" >> $slurmshell
            j=$((j+1))
        done
