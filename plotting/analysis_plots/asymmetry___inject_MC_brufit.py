import argparse
from tools___inject_brufit import *
ROOT.gROOT.ProcessLine(".x ~/link_to_clas12/packages/brufit_jupyter/macros/LoadBru.C")


parser = argparse.ArgumentParser(
                prog='asymmetry__inject_MC_brufit.py',
                description='Inject asymmetries into dihadron Monte Carlo and extract them (primarily using sWeighting technique)')
...
################################################################################################
# Program: asymmetry___inject_MC_brufit.py
# Author:  Gregory Matousek
# Date:    7/31/2023
#
# Purpose: Inject asymmetries into dihadron Monte Carlo and extract them (using MLM w/ BruFit)
...
def run_project_trial(i):
    ...
    project.create_injector(i)
    yaml_file = f"{project.project_loc}/trial_{i}.yaml"
    project.injector.run(yaml_file)
...
def main():
    ...
    project = InjectionBruProject(#project_title  = args.inject_project_name, # Injection project name
                               ...
                               n_cpus        = args.n_cpus,  # Max number of CPUs
                               injector_program_file = args.program)   
...
if __name__ == "__main__":
    ...
    infile = "/work/clas12/users/gmat/clas12/clas12_dihadrons/projects/pipi0_paper_RGA_only/volatile/data/piplus_pi0/MC_RGA_3051_0.root"
    io = DataIO(PROJECT_NAME="pipi0_paper_RGA_only",
                file_names=[args.infile],
                output_dir=f"inject_bru/{args.program.name.split('/')[-1]+'_'+pull_pair_from_string(args.infile)}")
