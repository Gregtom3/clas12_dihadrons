...
for ((i=0; i<${#pion_pairs[@]}; i++)); do
  ...
  for dataset in "${datasets[@]}"; do
    for ((binscheme=0; binscheme<$schemes; binscheme++)); do
        ...
        file=${pion_pair}_${dataset}_${binscheme}
...
EOF
        ...
        echo "/u/site/12gev_phys/2.4/Linux_CentOS7.7.1908-gcc9.2.0/root/6.20.04/bin/root $BRUFIT/macros/LoadBru.C -b -q -l $PWD/macros/calc_asymmetry.C\(\\\"${FILE}\\\",\\\"${BINNING_FILE}\\\",\\\"${BRU_DIR}\\\",$binscheme,\\\"${CUT_TITLE}\\\",0,1\)" >> $slurmshell

        echo "STANDARD: Submitting ${pion_pair} slurm job for $(basename "$FILE"), binning scheme $((binscheme+1)) of $((schemes))"
...
EOF
            ...
            echo "/u/site/12gev_phys/2.4/Linux_CentOS7.7.1908-gcc9.2.0/root/6.20.04/bin/root $BRUFIT/macros/LoadBru.C -b -q -l $PWD/macros/calc_asymmetry.C\(\\\"${FILE}\\\",\\\"${BINNING_FILE}\\\",\\\"${BRU_DIR}\\\",$binscheme,\\\"${CUT_TITLE}\\\",$ML,1,1,0,0\)" >> $slurmshell
            ...
            # SWEIGHTED FIT HERE
            # ---------------------------------------------------------------------------------------------
            file=sweight_${pion_pair}_${dataset}_${binscheme}_isML_$ML
...
EOF
            ...
            echo "/u/site/12gev_phys/2.4/Linux_CentOS7.7.1908-gcc9.2.0/root/6.20.04/bin/root $BRUFIT/macros/LoadBru.C -b -q -l $PWD/macros/calc_asymmetry.C\(\\\"${FILE}\\\",\\\"${BINNING_FILE}\\\",\\\"${BRU_DIR}\\\",$binscheme,\\\"${CUT_TITLE}\\\",$ML,0,0,1,0\)" >> $slurmshell2
            ...
            # SIDEBAND FIT HERE
            # ---------------------------------------------------------------------------------------------
            file=sideband_${pion_pair}_${dataset}_${binscheme}_isML_$ML
...
EOF
            ...
            echo "/u/site/12gev_phys/2.4/Linux_CentOS7.7.1908-gcc9.2.0/root/6.20.04/bin/root $BRUFIT/macros/LoadBru.C -b -q -l $PWD/macros/calc_asymmetry.C\(\\\"${FILE}\\\",\\\"${BINNING_FILE}\\\",\\\"${BRU_DIR}\\\",$binscheme,\\\"${CUT_TITLE}\\\",$ML,0,0,0,1\)" >> $slurmshell2
            ...
            if [[ $dataset == *"MC_"* ]]; then
                ...
                echo "/u/site/12gev_phys/2.4/Linux_CentOS7.7.1908-gcc9.2.0/root/6.20.04/bin/root $BRUFIT/macros/LoadBru.C -b -q -l $PWD/macros/calc_asymmetry.C\(\\\"${FILE}\\\",\\\"${BINNING_FILE}\\\",\\\"${BRU_DIR}\\\",$binscheme,\\\"${CUT_TITLE}\\\",$ML,0,1,0,0,1\)" >> $slurmshell
            fi
